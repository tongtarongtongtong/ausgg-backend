...
app.get('/api/admin/withdrawal-form', (req, res) => {
  res.json({
    fields: [
...
app.post('/api/admin/auth/login', (req, res) => {
  ...
  if (masterKey && masterKey === ADMIN_KEY) {
    return res.json({
      success: true,
      role: 'master',
      permissions: ['all'],
  ...
  if (!staff) {
    return res.status(403).json({ error: 'Invalid admin login' });
  }
